...
class YoloLoss(nn.Module):
    ...
    def get_contain_conf_loss(self, box_pred_response, box_target_response_iou):
        """
        Parameters:
        (response only pass in [c])
        box_pred_response : (tensor) size ( -1 , 1)
        box_target_response_iou : (tensor) size ( -1 , 1)
        Note : -1 corresponds to ravels the tensor into the dimension specified 
        See : https://pytorch.org/docs/stable/tensors.html#torch.Tensor.view_as
        ...
        ##### CODE #####
        contain_loss = F.mse_loss(box_pred_response, box_target_response_iou, reduction='sum')
        return contain_loss
    ...
    def get_no_object_loss(self, target_tensor, pred_tensor, no_object_mask):
        ...
        # hint1
        no_object_prediction = pred_tensor[no_object_mask].view(-1, T)
        no_object_target = target_tensor[no_object_mask].view(-1, T)
        ...
        # hint 3
        no_object_prediction_conf = no_object_prediction[no_object_prediction_mask]
        no_object_target_conf = no_object_target[no_object_prediction_mask]
        no_object_loss = F.mse_loss(no_object_prediction_conf, no_object_target_conf, reduction='sum')
    ...
    def find_best_iou_boxes(self, box_target, box_pred):
        ...
        contains_object_response_mask = torch.cuda.ByteTensor(box_target.size()).zero_()
        box_target_iou = torch.zeros(box_target.size()).cuda()
        
        for i in range(0, box_target.size()[0], B):
            # hint 4
            box_p = box_pred[i:i+B, :4]
            box_p_center = Variable(torch.FloatTensor(box_p.size()))
            box_p_center[:,:2] = box_p[:,:2] / float(S) - 0.5 * box_p[:,2:]
            box_p_center[:,2:] = box_p[:,:2] / float(S) + 0.5 * box_p[:,2:]
            
            box_t = box_target[i,:4].view(-1,4)
            box_t_center = Variable(torch.FloatTensor(box_t.size()))
            box_t_center[:,:2] = box_t[:,:2] / float(S) - 0.5 * box_t[:,2:]
            box_t_center[:,2:] = box_t[:,:2] / float(S) + 0.5 * box_t[:,2:]

            # hint 1 3
            iou = self.compute_iou(box_p_center, box_t_center)
            max_iou, max_idx = iou.max(0)
            max_idx = max_idx.data.cuda()
            
            # hint 2
            contains_object_response_mask[i + max_idx] = 1
    ...
    def forward(self, pred_tensor,target_tensor):
        ...
        contains_object_pred = pred_tensor[contains_object_mask].view(-1, T)
        bounding_box_pred = contains_object_pred[:,:B*5].contiguous().view(-1,5)
        classes_pred = contains_object_pred[:,B*5:]
        ...
        ##### CODE #####
        contains_object_target = target_tensor[contains_object_mask].view(-1, T)
        bounding_box_target = contains_object_target[:,:B*5].contiguous().view(-1,5)
        classes_target = contains_object_target[:,B*5:]
        ...
        contain_loss = self.get_contain_conf_loss(box_prediction_response[:,4], box_target_response_iou[:,4])
        regression_loss = self.get_regression_loss(box_prediction_response[:,:4], box_target_response[:,:4])
